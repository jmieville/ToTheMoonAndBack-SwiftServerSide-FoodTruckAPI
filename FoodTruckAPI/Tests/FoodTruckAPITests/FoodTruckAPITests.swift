...
class FoodTruckAPITests: XCTestCase {
    ...
    func testCountReviewsForTruck() {
        ...
        // Add truck
        trucks.addTruck(name: "12345", foodType: "testReview", avgCost: 0, latitude: 0, longitude: 0) { (addedTruck, err) in
            guard err == nil else {
                XCTFail()
                return
            }
            
            guard let addedTruck = addedTruck else {
                XCTFail()
                return
            }
            // Add reviews for fictitious truck id 12345
            trucks.addReview(truckId: addedTruck.docId, reviewTitle: "testReview", reviewText: "testReview", reviewStarRating: 1) { (review, err) in
                guard err == nil else {
                    XCTFail()
                    return
                }
                // add 2nd review
                trucks.addReview(truckId: addedTruck.docId, reviewTitle: "testReview2", reviewText: "testReview2", reviewStarRating: 2) { (review, err) in
                    guard err == nil else {
                        XCTFail()
                        return
                    }
                    
                    trucks.countReviews(truckId: addedTruck.docId, completion: { (count, err) in
                        guard err == nil else {
                            XCTFail()
                            return
                        }
    ...
    func testGetAverageStarRating() {
        ...
        // add some reviews for fictionious truck
        // Star ratings 5 and 2 which should give average of 3.5 which should be roudned to 4
        trucks.addReview(truckId: "23456", reviewTitle: "testReview", reviewText: "testReview", reviewStarRating: 5) { (revie1, err) in
            guard err == nil else {
                XCTFail()
                return
            }
            ...
            trucks.addReview(truckId: "23456", reviewTitle: "testReview", reviewText: "testReview", reviewStarRating: 2, completion: { (review, err) in
                ...
                trucks.averageRating(truckId: "23456", completion: { (rating, err) in
                    ...
                    // add another 3 star review which would giev average pf 3.333 which will be rounded to 3
